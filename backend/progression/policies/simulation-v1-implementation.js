...
function contribution(event, decision) {
  const a = decision.signals; const factor = decision.factor;
  ...
  let personal = (baseByType[event.eventType] || 1) * quality; const specialties = {};
  ...
  if (event.activityClass === 'TRANSACT') { const dollars = Number(event.economic.amountMinor) / 100; const breadth = Math.log2(1 + Math.max(1, a.uniqueSupporters || 1)); personal = 2 + Math.min(10, Math.log10(1 + dollars) * 2) + breadth * 2; specialties.economy = personal; specialties.creator = personal * 0.6; }
  ...
  personal *= factor; for (const key of Object.keys(specialties)) specialties[key] *= factor;
  const crossFactionInfluence = Math.log2(1 + Math.max(0, a.uniqueFactions || 0)) * Math.log2(1 + Math.max(0, a.uniquePeople || 0)) * factor;
  const diversity = clamp(a.engagementDiversity ?? 0.5, 0, 1);
  const faction = event.affiliations.beneficiary.state === 'affiliated' ? (personal * 0.28 + crossFactionInfluence * 1.7) * (0.5 + diversity) : 0;
  return { personal, specialties, crossFactionInfluence, faction, publicExplanationCategories: crossFactionInfluence > 4 ? ['CROSS_COMMUNITY_REACH'] : [] };
}
...
module.exports = Object.freeze({ evaluate, contribution });
