...
const { dedicatedWorkerEnabled, livePipelineEnabled, policyIdentity, processingScope } = require('../progression/operations/config');
...
async function main() {
  ...
  if (!livePipelineEnabled()) throw new Error('PROGRESSION_LIVE_PIPELINE_ENABLED must be explicitly true');
  if (!process.env.MONGODB_URI) throw new Error('MONGODB_URI is required');
  await mongoose.connect(process.env.MONGODB_URI, { autoIndex: false, autoCreate: false });
  const stop = startShadowWorker({ intervalMs: process.env.PROGRESSION_WORKER_POLL_MS, batchSize: process.env.PROGRESSION_WORKER_BATCH_SIZE, concurrency: process.env.PROGRESSION_WORKER_CONCURRENCY, maxAttempts: process.env.PROGRESSION_WORKER_MAX_ATTEMPTS, policyIdentity: policyIdentity(), processingScope: processingScope() });
