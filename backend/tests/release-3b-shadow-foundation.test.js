...
test('subject rebuild persists personal state only and remains idempotent', async () => {
  const policyIdentity = { policyId: policy.artifact.policyId, version: policy.artifact.version, artifactDigest: policy.artifact.artifactDigest };
  ...
  assert.equal(await Projection.countDocuments({ scope: 'faction', subjectId: 'Neon', projectionContextId: first.projectionContext.projectionContextId }), 0);
});
...
test('subject rebuild transaction does not leave partial decision/contribution/personal state', async () => {
  const policyIdentity = { policyId: policy.artifact.policyId, version: policy.artifact.version, artifactDigest: policy.artifact.artifactDigest };
  ...
  try {
    repository.storeProjection = async (value, options) => {
      if (value.scope === 'personal') throw new Error('simulated personal projection failure');
      return originalStoreProjection(value, options);
    };
    await assert.rejects(service.rebuildSubjectProjection({ userId: 'cross_faction_viral', policyIdentity }), /simulated personal projection failure/);
  } finally {
