...
const MoodStatus: React.FC<{ mood?: string }> = ({ mood }) => {
  const moods = [
  ...
  const [defaultMood] = useState(moods[Math.floor(Math.random() * moods.length)]);
  const displayMood = mood || defaultMood.text;
  const emoji = moods.find(m => m.text === displayMood)?.emoji || '⚡';

  return (
    <div className="flex items-center gap-2 bg-black/40 px-3 py-1 rounded border border-[#39FF14]/30">
      <span className="text-lg">{emoji}</span>
      <span className="text-[10px] font-mono text-[#39FF14]">{displayMood}</span>
    </div>
  );
};
    ...
    <div className="mt-6 space-y-3">
      <div className="grid grid-cols-2 gap-2 text-[10px] font-mono">
        ...
        <div className="text-white">
          {user.memberSince ? new Date(user.memberSince).toLocaleDateString() : 'UNKNOWN'}
        </div>
        <div className="text-gray-500">VERIFIED:</div>
        <div className={user.isVerified ? 'text-[#39FF14]' : 'text-gray-500'}>
          {user.isVerified ? '✓ VERIFIED' : 'UNVERIFIED'}
        </div>
        <div className="text-gray-500">AGE VERIFIED:</div>
        <div className={user.isAgeVerified ? 'text-[#FF00FF]' : 'text-gray-500'}>
          {user.isAgeVerified ? '✓ 18+ CONFIRMED' : 'NOT VERIFIED'}
        </div>
      </div>
      ...
      {user.interests && user.interests.length > 0 && (
        <div className="flex flex-wrap gap-2 pt-2">
          {user.interests.map((interest, i) => (
            <span key={i} className="text-[9px] px-2 py-1 bg-[#39FF14]/20 text-[#39FF14] border border-[#39FF14]/30 rounded">
              #{interest}
            </span>
          ))}
        </div>
...
const ContactButtons: React.FC<{ onMessage?: () => void; onAddFriend?: () => void; isFollowing?: boolean }> = ({ onMessage, onAddFriend, isFollowing }) => (
  <div className="grid grid-cols-2 gap-2">
    ...
    <button
      onClick={onAddFriend}
      className={`flex items-center justify-center gap-2 px-4 py-3 border-2 transition-all group ${
        isFollowing
          ? 'bg-[#FF00FF] border-[#FF00FF] text-black'
          : 'bg-[#FF00FF]/10 border-[#FF00FF] text-[#FF00FF] hover:bg-[#FF00FF] hover:text-black'
      }`}
    >
      <UserPlus size={16} className="group-hover:scale-110 transition-transform" />
      <span className="text-[10px] font-bold font-mono">{isFollowing ? 'FOLLOWING' : 'FOLLOW'}</span>
    </button>
  </div>
);
    ...
    >
      ...
      <div className="fixed inset-0 z-[1] pointer-events-none">
        {!localTheme.backgroundImage && (
          <div className="absolute inset-0" style={{ backgroundColor: localTheme.backgroundColor || '#0a0a0a' }} />
        )}
        {localTheme.backgroundImage && (
          <div
        ...
        <div className="flex items-center gap-4">
          <div>
            <h2 className="text-xl font-bold text-[#39FF14] tracking-widest uppercase flex items-center gap-2">
              ...
              {user.username}'s Holo-Deck
              {isEditing && <Layers size={16} className="animate-bounce text-[#FF00FF]" />}
            </h2>
            <div className="flex items-center gap-3 mt-1">
              <p className="text-[10px] text-gray-500 font-mono">
                FACTION: <span className="text-[#39FF14]">{user.faction || 'UNAFFILIATED'}</span>
              </p>
              <VisitorCounter />
            </div>
          </div>
          <MoodStatus mood={user.mood} />
        </div>
      ...
      {!isHudVisible && (
        <button onClick={() => setIsHudVisible(true)} className="fixed top-24 right-4 z-30 bg-black/60 backdrop-blur border-2 border-[#39FF14] text-[#39FF14] p-3 rounded-full shadow-[0_0_20px_#39FF14] animate-bounce">
          <Eye size={24} />
        </button>
      )}
            ...
            <div className="bg-black/80 backdrop-blur-md border-2 border-[#39FF14] p-4 relative group">
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                <div className="bg-black/50 p-2 border border-gray-800">
                  <div className="text-[#FF00FF] text-lg font-bold">{user.followingCount ?? 0}</div>
                  <div className="text-[8px] text-gray-500 font-mono">FOLLOWING</div>
                </div>
                <div className="bg-black/50 p-2 border border-gray-800">
                  <div className="text-[#39FF14] text-lg font-bold">{user.followersCount ?? 0}</div>
                  <div className="text-[8px] text-gray-500 font-mono">FOLLOWERS</div>
                </div>
                <div className="bg-black/50 p-2 border border-gray-800">
                  <div className="text-[#00FFFF] text-lg font-bold">{user.postsCount ?? 0}</div>
          ...
          <div className="space-y-4">
            ...
            <div className="h-48 group">
              <div className="w-full h-full bg-black/80 backdrop-blur-md border-2 border-[#39FF14] p-4 flex flex-col justify-center relative overflow-hidden">
                <div className="absolute top-0 left-0 bg-[#39FF14] text-black text-[9px] font-bold px-2 py-0.5 flex items-center gap-1">
                  <Wallet size={10} /> CRYPTO_VAULT
                </div>
                <div className="mt-2 flex flex-col gap-1">
                  <span className="text-[10px] text-gray-500 uppercase">ETH Address</span>
                  <div className="flex items-center gap-2 bg-black border border-gray-800 p-2 font-mono text-[10px] text-[#39FF14]">
                    <span className="truncate">{user.walletAddress || 'NOT CONNECTED'}</span>
                    <button className="hover:text-white transition-colors"><Copy size={10} /></button>
                  </div>
                </div>
                {user.btcAddress && (
                  <div className="mt-1 flex flex-col gap-1">
                    <span className="text-[10px] text-gray-500 uppercase">BTC Address</span>
                    <div className="flex items-center gap-2 bg-black border border-gray-800 p-2 font-mono text-[10px] text-orange-500">
                      <span className="truncate">{user.btcAddress}</span>
                      <button className="hover:text-white transition-colors"><Copy size={10} /></button>
                    </div>
                  </div>
                )}
