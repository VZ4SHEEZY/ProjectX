...
const AuthPage: React.FC<AuthPageProps> = ({ onLoginSuccess }) => {
  ...
  // Coinbase Smart Wallet login handler
  const handleCoinbaseLogin = async () => {
    try {
      alert("Connecting to Coinbase Smart Wallet...")
    } catch (error) {
      console.error("Coinbase login failed:", error)
    }
  }
      ...
      <div className="relative z-30 w-full max-w-md p-6 animate-in zoom-in-95 duration-1000">
        <div className="relative bg-black/40 backdrop-blur-xl border border-[#39FF14]/30 p-8 shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {/* Coinbase Smart Wallet Login - Only show if age verified */}
            {!isAgeVerified && (
              <div className="text-[#FF00FF] text-[10px] font-bold tracking-widest text-center mt-3 p-2 bg-[#FF00FF]/10 border border-[#FF00FF]/20 rounded-lg">
                🚨 AGE VERIFICATION REQUIRED TO ACCESS MONETIZATION
              </div>
            )}
            ...
            {isAgeVerified ? (
              <button
                ...
                className="w-full mt-3 py-2 bg-[#0050D6] hover:bg-[#003db3] text-white font-bold tracking-widest rounded-lg transition-all flex items-center justify-center gap-2"
              >
